  ...
  return (
    <Transition className="flex flex-col items-center h-full shadow-2xl rounded-lg xl:mx-auto p-1 font-quicksand relative">
        ...
        <div className="flex flex-col">
          ...
          {CityId && (
            <span className="text-gray-400 text-sm font-extralight">
              {getOneCity(CityId).city + " - " + getOneCity(CityId).barrio}
            </span>
          )}
        </div>
      ...
      <div className="flex flex-col justify-center mt-2">
        ...
        >
          <div
            className={`w-[50px] p-1 px-2 rounded absolute top-0 right-0 text-white ${
              status == "rent" ? "bg-green-400" : "bg-red-400"
            }`}
          >
            <span>{status}</span>
          </div>
        ...
        <div className="flex gap-2 justify-center">
          <div className="flex w-[50px] px-2 my-2 py-1 rounded-lg bg-slate-300/30">
            <LiaBathSolid />
            <span className="ml-2 text-xs block">{bathrooms}</span>
          </div>
          <div className="flex w-[50px] px-2 my-2 py-1 rounded-lg bg-slate-300/30">
            <LiaBedSolid />
            <span className="ml-2 text-xs block">{bedrooms}</span>
          </div>
          <div className="flex w-[50px] px-2 my-2 py-1 rounded-lg bg-slate-300/30">
            <LiaRulerCombinedSolid />
            <span className="ml-2 text-xs block">{size}</span>
          </div>
        </div>
        <div className="min-h-[200px] xl:min-h-0 max-w-[400px]">
          <span className="text-xs  my-2 block text-gray-400 font-extralight">
            Description:
          </span>
        ...
        <div
          onClick={() => submit(input, input.id)}
          className="text-white flex justify-center absolute bottom-3 right-[calc(50%-100px)] "
        >
