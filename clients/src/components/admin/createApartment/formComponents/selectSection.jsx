...
function SelectSection({name, value, label, handle}) {
    ...
    return ( 
        <div className="flex flex-col mx-2 my-2">
            ...
            <div onClick={toogleOpen} className="cursor-pointer max-w-[170px] relative flex border p-[3px] rounded-md border-black ">
                ...
                <div  className={`${openStatus ? 'absolute' : 'hidden'} rounded-sm z-100 bg-white w-full left-0 top-[25px] border`}>
                    {
                        city && city.map(e => {
                            ...
                            return <button key={e.id} name={name} onClick={(e)=> handle(e)} value={e.id} className="p-1 hover:cursor-pointer w-full block hover:bg-gray-300 text-gray-400 text-[12px]"  >{`${e.city} - ${e.barrio}`}</button>
                        })
                    }
                </div>
            </div>
